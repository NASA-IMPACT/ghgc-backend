"""CDK Construct for ghgc-backend RDS instance."""
import json
import os
...
class BootstrapPgStac(Construct):
    ...
    def __init__(
        ...
        pgstac_version = db_settings.pgstac_version
        schema_version = db_settings.schema_version
...
class RdsConstruct(Construct):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        publicly_accessible = False if db_settings.private_subnets else True
        subnet_type = (
            aws_ec2.SubnetType.PRIVATE_ISOLATED
            if db_settings.private_subnets is True
        ...
        parameter_group = aws_rds.ParameterGroup(
            ...
            engine=engine,
            parameters={
                "max_locks_per_transaction": db_settings.max_locks_per_transaction,
                "work_mem": db_settings.work_mem,
                "temp_buffers": db_settings.temp_buffers,
            },
        )

        # Create a new database instance from snapshot if provided
        if db_settings.snapshot_id:
            ...
            credentials = aws_rds.SnapshotCredentials.from_generated_secret(
                username=db_settings.admin_user
            )

            database = aws_rds.DatabaseInstanceFromSnapshot(
                self,
                id="rds",
                snapshot_identifier=db_settings.snapshot_id,
