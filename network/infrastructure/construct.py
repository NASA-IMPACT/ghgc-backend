"""
CDK construct for ghgc-backend VPC.
"""
...
class VpcConstruct(Construct):
    ...
    def __init__(
        ...
        # Or create a new VPC using the deployment stage configuration
            ...
            if stage == "prod":
                vpc_settings = prod_vpc_settings
            elif stage == "staging":
                vpc_settings = staging_vpc_settings
            else:
                vpc_settings = dev_vpc_settings

            public_subnet = aws_ec2.SubnetConfiguration(
                name="public",
                subnet_type=aws_ec2.SubnetType.PUBLIC,
                cidr_mask=vpc_settings.public_mask,
            )
            private_subnet = aws_ec2.SubnetConfiguration(
                name="private",
                subnet_type=aws_ec2.SubnetType.PRIVATE_WITH_NAT,
                cidr_mask=vpc_settings.private_mask,
            )

            self.vpc = aws_ec2.Vpc(
                self,
                "vpc",
                max_azs=vpc_settings.max_azs,
                cidr=vpc_settings.cidr,
                subnet_configuration=[public_subnet, private_subnet],
                nat_gateways=vpc_settings.nat_gateways,
            )
